...
export const analyzeSalesData = async (records: SaleRecord[]): Promise<string> => {
  ...
  const prompt = `
    ...
    In your report:
    1. Agent Performance: Audit pricing consistency and volume for each agent.
    2. Pricing Anomalies: Identify any transactions that seem outside the market norm.
    3. Customer Insights: Identify repeat customers or significant buyer trends that field agents should follow up on.
