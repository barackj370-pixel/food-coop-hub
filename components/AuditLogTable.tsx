            ...
            <div key={groupKey} className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-lg overflow-x-auto">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {records.filter(r => r.isAggregate).map(r => (
                  ...
                  {records.filter(r => !r.isAggregate).length > 0 && (
                    <tr className="text-[11px] font-bold bg-slate-50/30">
                      <td className="py-6 text-slate-500 font-bold italic">{groupBy === 'date' ? groupKey : groupBy === 'cluster_and_date' ? groupKey.split(' | ')[1] : groupKey}</td>
                      <td className="py-6 text-slate-500 italic">Auto-Consolidated Individual Orders</td>
                      <td className="py-6 text-slate-500 italic">Various</td>
                      <td className="py-6 text-slate-500 italic">-</td>
                      <td className="py-6 text-slate-500 italic">-</td>
                      <td className="py-6 font-black text-black">KSh {records.filter(r => !r.isAggregate).reduce((sum, r) => sum + Number(r.totalSale), 0).toLocaleString()}</td>
                      <td className="py-6 font-black text-green-600">KSh {records.filter(r => !r.isAggregate).reduce((sum, r) => sum + Number(r.coopProfit), 0).toLocaleString()}</td>
                      <td className="py-6 text-right">
                        <span className="px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest bg-slate-200 text-slate-600">AGGREGATED</span>
                      </td>
                    </tr>
                  )}
                </tbody>
