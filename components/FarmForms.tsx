...
const FarmForms: React.FC<FarmFormsProps> = ({ agentIdentity, dynamicClusters, users = [] }) => {
  ...
  useEffect(() => {
    supabase.from('farm_baselines').select('*').eq('farmer_phone', agentIdentity.phone)
      .then(({ data }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error("Unable to capture GPS. Please ensure location services are enabled.");
      }
      ...
      if (farmBaselines.length > 0 && location) {
        ...
        if (!isVerified) {
          throw new Error("GPS mismatch: You do not appear to be standing on any of your registered farms.");
        }
      ...
      setSubmitStatus({ type: 'success', message: 'Form submitted successfully! GPS location verified.' });
      form.reset();
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <datalist id="homestead-list">
        {users.filter(u => u.homesteadName || u.name).map((u, i) => (
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {submitStatus && (
              <div className={`col-span-1 md:col-span-2 p-4 rounded-xl text-sm font-bold flex items-center gap-3 ${
          ...
          {activeForm === 'solidarity' ? (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="space-y-6 pt-6 border-t border-slate-100">
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-4">Homestead Owner</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 font-bold text-slate-700 outline-none focus:bg-white focus:border-emerald-400 transition-all"
                      onChange={(e) => {
                        const match = users.find(u => (u.homesteadName || u.name) === e.target.value);
                        if (match) {
                          const contactInput = e.target.form?.elements.namedItem('homesteadVisitedContact') as HTMLInputElement;
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Name of Homestead Visited</label>
                  <input type="text" name="homesteadVisitedName" list="homestead-list" required placeholder="Enter homestead name" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 font-bold text-slate-700 outline-none focus:bg-white focus:border-emerald-400 transition-all" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Name of Homestead Convener</label>
                  <input type="text" name="convenerName" list="homestead-list" required placeholder="Enter convener name" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 font-bold text-slate-700 outline-none focus:bg-white focus:border-emerald-400 transition-all" onChange={(e) => {
                    const match = users.find(u => (u.homesteadName || u.name) === e.target.value);
                    if (match) {
                      const contactInput = e.target.form?.elements.namedItem('convenerContact') as HTMLInputElement;
                      if (contactInput) contactInput.value = match.phone;
                      // Autofill homestead name if it's the same
                      const nameInput = e.target.form?.elements.namedItem('homesteadVisitedName') as HTMLInputElement;
                      if (nameInput && !nameInput.value && match.homesteadName) nameInput.value = match.homesteadName;
                    }
                  }} />
                </div>
                <div className="space-y-2">
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input type="text" name="homesteadName" list="homestead-list" required placeholder="Enter household or homestead name" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 font-bold text-slate-700 outline-none focus:bg-white focus:border-emerald-400 transition-all" onChange={(e) => {
                    const match = users.find(u => (u.homesteadName || u.name) === e.target.value);
                    if (match) {
                      const contactInput = e.target.form?.elements.namedItem('homesteadContact') as HTMLInputElement;
                      if (contactInput) contactInput.value = match.phone;
                    }
                  }} />
                </div>
              ...
              <div className="space-y-6 pt-6 border-t border-slate-100">
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-4">Food Coop Marketplace</h3>
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">List the types of Farm Food, Processed Food and None Food Products to order from Food Coop this week</label>
                    <textarea name="foodConsumed" rows={3} required placeholder="List products to order this week..." className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 font-bold text-slate-700 outline-none focus:bg-white focus:border-emerald-400 transition-all resize-none"></textarea>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">List products to supply Food Coop this week</label>
                    <textarea name="productsNeeded" rows={3} required placeholder="List products to supply..." className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 font-bold text-slate-700 outline-none focus:bg-white focus:border-emerald-400 transition-all resize-none"></textarea>
                  </div>
                </div>
              </div>
            </div>
