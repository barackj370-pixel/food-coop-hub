...
const App: React.FC = () => {
  ...
  const handlePlaceOrder = async (listing: ProduceListing, qty: number) => {
    ...
    try { await syncOrderToCloud(newOrder); alert("Order placed successfully! A field agent will contact you for delivery and payment."); } catch (err) { console.error("Order sync failed:", err); }
  };
  ...
  // Added handleUseProduceListing to fix "Cannot find name 'handleUseProduceListing'" error
  const handleUseProduceListing = (listing: ProduceListing) => {
    setCurrentPortal('MARKET');
    setMarketView('SALES');
    setFulfillmentData({
      cropType: listing.cropType,
      unitsSold: listing.unitsAvailable,
      unitType: listing.unitType,
            ...
            <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-xl">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b pb-4">
                    <tr><th>Date</th><th>Participants</th><th>Commodity</th><th>Commission</th><th className="text-right">Action</th></tr>
                  </thead>
                  <tbody className="divide-y">
                    {filteredRecords.filter(r => r.status === RecordStatus.DRAFT).map(r => (
                      <tr key={r.id} className="hover:bg-slate-50/50">
                        <td className="py-6 font-bold">{r.date}</td>
                        <td className="py-6"><div className="text-[9px] space-y-1 uppercase font-bold text-slate-500"><p className="text-black">Agent: {r.agentName}</p><p>Supplier: {r.farmerName}</p><p>Buyer: {r.customerName}</p></div></td>
                        <td className="py-6 uppercase font-bold">{r.cropType}</td>
                        <td className="py-6 font-black text-green-600">KSh {Number(r.coopProfit).toLocaleString()}</td>
                        <td className="py-6 text-right"><button type="button" onClick={() => handleUpdateStatus(r.id, RecordStatus.PAID)} className="bg-green-500 text-white px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-green-600 shadow-md">Confirm Receipt</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="space-y-8 animate-in fade-in duration-300">
            <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-xl">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b pb-4">
                    <tr><th>Details</th><th>Participants</th><th>Financials</th><th className="text-right">Action</th></tr>
                  </thead>
                  <tbody className="divide-y">
                    {filteredRecords.filter(r => r.status === RecordStatus.PAID || r.status === RecordStatus.VALIDATED).map(r => (
                      <tr key={r.id} className="hover:bg-slate-800/50">
                        ...
                        <td className="py-6"><div className="text-[9px] space-y-1 uppercase font-bold text-slate-500"><p className="text-black">Agent: {r.agentName}</p><p>Supplier: {r.farmerName}</p></div></td>
                        <td className="py-6 font-black text-black"><p>Gross: KSh {Number(r.totalSale).toLocaleString()}</p></td>
                        <td className="py-6 text-right">{r.status === RecordStatus.PAID ? (<button onClick={() => handleUpdateStatus(r.id, RecordStatus.VALIDATED)} className="bg-black text-white px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest">Verify</button>) : (<button onClick={() => handleUpdateStatus(r.id, RecordStatus.VERIFIED)} className="bg-red-600 text-white px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest">Audit Seal</button>)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
