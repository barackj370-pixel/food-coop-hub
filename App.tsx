...
const App: React.FC = () => {
  // CRITICAL: Initialize from local storage to prevent data loss on refresh
  ...
  const loadCloudData = useCallback(async () => {
    ...
    try {
      ...
      if (cloudRecords) {
        setRecords(prev => {
          // Merge logic: keep local drafts that aren't in the cloud yet
  ...
  const filteredRecords = useMemo(() => {
    ...
    // System Developer and privileged roles see EVERYTHING
    if (!isSystemDev && agentIdentity) {
      const isPrivileged = agentIdentity.role === SystemRole.MANAGER || 
                           agentIdentity.role === SystemRole.FINANCE_OFFICER ||
                           agentIdentity.role === SystemRole.AUDITOR;
      
      if (!isPrivileged) {
        // Field Agents only see their own records
  ...
  const handleExportDetailedCsv = () => {
    ...
    const headers = ["Date", "Agent", "Cluster", "Supplier", "Buyer", "Commodity", "Units", "Unit Price", "Gross Sale", "Commission", "Status", "ID"];
    const rows = filteredRecords.map(r => [
      r.date, r.agentName, r.cluster, r.farmerName, r.customerName, r.cropType, `${r.unitsSold} ${r.unitType}`, r.unitPrice, r.totalSale, r.coopProfit, r.status, r.id
    ]);
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute("href", url);
    link.setAttribute("download", `kpl_full_audit_report_${new Date().toISOString().split('T')[0]}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
        ...
        <tbody className="divide-y divide-slate-50">
          {data.map(r => (
            <tr key={r.id} className="text-[11px] font-bold group hover:bg-slate-50/50">
              ...
              <td className="py-6">
                 <p className="text-black uppercase text-[10px]">S: {r.farmerName}</p>
                 <p className="text-slate-500 uppercase text-[9px]">C: {r.customerName}</p>
                 <p className="text-slate-400 uppercase text-[8px]">Agent: {r.agentName} ({r.cluster})</p>
              </td>
              ...
              <td className="py-6 text-right">
                <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                  ...
                  r.status === 'VALIDATED' ? 'bg-amber-100 text-amber-700' :
            ...
            <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-xl">
               <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
                  ...
                  <div className="flex gap-4">
                    <button onClick={handleExportSummaryCsv} className="bg-slate-100 text-black px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all"><i className="fas fa-chart-pie mr-2"></i> Summary CSV</button>
                    <button onClick={handleExportDetailedCsv} className="bg-black text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-slate-900 transition-all"><i className="fas fa-file-invoice mr-2"></i> Detailed Audit Report</button>
                  </div>
               </div>
