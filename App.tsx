...
// Fix: Added export to resolve import error in index.tsx
export const App: React.FC = () => {
  ...
  const handleUseProduceListing = (listing: ProduceListing) => {
    ...
    setFulfillmentData({
      ...
      // Reset variables not relevant to the new supplier-initiated sale
      unitsSold: 0,
      customerName: '',
      customerPhone: ''
    });
                        ...
                        <tbody className="divide-y divide-slate-800">
                           {filteredOrders.filter(o => o.status === OrderStatus.OPEN).map(o => (
                             <tr key={o.id} className="hover:bg-slate-800/50">
                               ...
                               <td className="py-4">
                                  {agentIdentity.role !== SystemRole.SUPPLIER && (
                                    <button type="button" onClick={(e) => { e.stopPropagation(); handleFulfillOrderClick(o); }} className="bg-white text-black px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-slate-200 shadow-md flex items-center gap-2">
